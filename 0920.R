...
# 막대 그래프(1. 그래프의 데이터 초기화, 2. 그래프모양, 3. 제목과 라벨 설정)
...
ggplot(data = df1, aes(x=부서))+
  geom_bar(stat="count")+
  labs(title="부서별 영업실적", x="부서명", y="매출건 수")
...
ggplot(data = df1, aes(x=부서, y=매출))+
  geom_bar(stat="identity", fill = c("yellow"), color = "black")+
  labs(title="부서별 영업실적", x="부서명", y="매출(억원)")

# 막대마다 사용자가 지정한 색상으로 변경
ggplot(data = df1, aes(x=부서, y=매출))+
  geom_bar(stat="identity", fill = c("green", "yellow", "gold"), color = "black")+
  labs(title="부서별 영업실적", x="부서명", y="매출(억원)")

# 막대에 레인보우 색상 중 눈에 잘띄는 색상으로 자동 변경
ggplot(data = df1, aes(x=부서, y=매출))+
...
# 부서별 분기별 막대그래프 작성(조건1 => 3분기 실적을 추가, 
# 조건2 => gdRdriver 패키지의 팔레트에 있는 색상을 막대에 설정, 
...
ggplot(data = df, aes(x=부서, y=매출, fill = 분기))+
  geom_bar(stat="identity")+
  scale_fill_manual(values = c("red", "magenta", "blue"))+
  labs(title="부서별 분기별 영업실적", x="부서명", y="영업매출(억원)")
...
ggsave("D:/박효제/2-2/R_Script/graph_image/graph1.png", width = 10, height = 13, units = "cm")
...
ggplot(data = women, aes(x=height, y=weight))+
  geom_point()+
  labs(title = "미국 30~39세 여성의 키와 몸무게의 상관관계", x="키(inch)", y="몸무게(pound)")

# 점의 모양, 색상, 크기를 설정
ggplot(data = women, aes(x=height, y=weight))+
  geom_point(shape = 15, color = "tomato", size =)+
  labs(title = "미국 30~39세 여성의 키와 몸무게의 상관관계", x="키(inch)", y="몸무게(pound)")

...
Y = c(12, 20, NULL, 40, 50, NaN, 70, 80, 90, 100, 110)
...
ggpairs(data=iris, columns = 1:4, upper = list(continuous=wrap("cor", size=2.)))

ggpairs(data=iris, columns = 1:4, aes(color=Species, alpha=0.5), upper = list(continuous=wrap("cor", size=2.)))
...
# df의 W열이 빈 문자열이 아니고, Z열의 유한한 값인 행만 추출
df2 = filter(df, nzchar(W) & is.finite(Z))
df2

# df의 W열이 빈 문자열이 아니고, Z열의 유한한 값인 행과 특정열(W열, X열) 추출
df2 = subset(df, nzchar(W) & is.finite(Z), select = c(W, X))

# 결측치나 특정 조건을 만족하는 값들을 업데이트(수정)하는 방법
df3 = df
df3$X[df3$X==0] = 10
df3$X[is.na(df3$X)] = 4 # not arayable
df3$Y[is.nan(df3$Y)] = 60 # not a number
df3$Z[is.nan(df3$Z)] = 30
df3$Z[df3$Z>=Inf] = 10 # 크거나 같으면 양의 infinity를 의미
df3$Z[df3$Z<=-Inf] = 20
df3$W[df3$W==""] = "Unknown"
...
df3$X[df3$X==0] = 10
# X열을 평균값으로 수정
df3$X[is.na(df3$X)] = mean(df3$X[is.finite(df3$X)]) # not arayable
# Y열의 nan을 중앙값으로 수정
df3$Y[is.nan(df3$Y)] = median(df3$Y[is.finite(df3$Y)]) # not a number
# Z열 nan을 평균값으로 수정
df3$Z[is.nan(df3$Z)] = mean(df3$Z[is.finite(df3$Z)])
df3$Z[df3$Z>=Inf] = max(df3$Z[is.finite(df3$Z)]) # 크거나 같으면 양의 infinity를 의미
